...
# スクレイピングするURL
PAGE_URL_BASE = "https://kakaku.com/pc/pc-memory/itemlist.aspx?pdf_Spec101=6,7&pdf_Spec102=1&pdf_pg="
...
INSERT_SQL = """
  INSERT INTO memory
  (product_id, url, product_name, price, release_date, capacity, count, spec)
  VALUES
  (%s, %s, %s, %s, %s, %s, %s, %s)
"""
...
def extract_data_from_soup(soup):
  ...
  for i in range(0, PAGE_CONTENT_COUNT):
    try:
      ...
      count = elems[3*i+3].select("td")[9].get_text()
      spec = elems[3*i+3].select("td")[10].get_text().replace(" ", "").replace("SDRAM", "")
      data = {
        "product_id"  : id,
        "url"         : link,
