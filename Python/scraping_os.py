...
# スクレイピングするURL
PAGE_URL_BASE = "https://kakaku.com/pc/os-soft/itemlist.aspx?pdf_pg="

# 取得するページ数
MAX_PAGE = 1

# 取得するページの製品数
PAGE_CONTENT_COUNT = 5
...
INSERT_SQL = """
  INSERT INTO os
  (product_id, url, product_name, price, release_date)
  VALUES
  (%s, %s, %s, %s, %s)
"""

# 削除クエリ
DELETE_SQL = "DELETE FROM os"
...
def extract_data_from_soup(soup):
  ...
  for i in range(0, PAGE_CONTENT_COUNT):
    try:
      id = elems[2*i+3].select("td")[0].select("a")[0]["href"].replace('https://kakaku.com/item/', '').replace('/', '')
      link = elems[2*i+3].select("td")[0].select("a")[0]["href"]
      price = elems[2*i+3].select("td")[1].select("ul")[0].select("li")[0].select("a")[0].get_text().replace('¥', '').replace(',', '')
      release = elems[2*i+3].select(".swdate1")[0].get_text().replace(' ', '')
      name = elems[2*i+2].select("td")[1].select("a")[0].get_text().replace('　', ' ')
      data = {
        "product_id"  : id,
        "url"         : link,
