...
# スクレイピングするURL
PAGE_URL_BASE = "https://kakaku.com/pc/ssd/itemlist.aspx?pdf_Spec102=10,2,3,6,7,8,9&pdf_Spec104=1&pdf_pg="
...
INSERT_SQL = """
  INSERT INTO ssd
  (product_id, url, product_name, price, release_date, capacity, spec_size, interface)
  VALUES
  (%s, %s, %s, %s, %s, %s, %s, %s)
"""

# 削除クエリ
DELETE_SQL = "DELETE FROM ssd"
...
def extract_data_from_soup(soup):
  ...
  for i in range(0, PAGE_CONTENT_COUNT):
    try:
      ...
      spec_size = elems[3*i+3].select("td")[11].get_text()
      interface = elems[3*i+3].select("td")[12].get_text().replace("PCI-Express", "PCIe").replace("Serial ATA","SATA")
      data = {
        "product_id"  : id,
        "url"         : link,
