...
# スクレイピングするURL
PAGE_URL_BASE = "https://kakaku.com/pc/motherboard/itemlist.aspx?pdf_pg="
...
INSERT_SQL = """
  INSERT INTO mb
  (product_id, url, product_name, price, release_date, formfactor, chipset, socket, ram_spec)
  VALUES
  (%s, %s, %s, %s, %s, %s, %s, %s, %s)
"""

# 削除クエリ
DELETE_SQL = "DELETE FROM mb"
...
def extract_data_from_soup(soup):
  ...
  for i in range(0, PAGE_CONTENT_COUNT):
    try:
      ...
      formfactor = elems[3*i+3].select("td")[8].get_text()
      socket = elems[3*i+3].select("td")[9].get_text()
      elems[3*i+3].select("td")[10].find("br").replace_with(" ")
      chipset = elems[3*i+3].select("td")[10].get_text()
      ram_spec = elems[3*i+3].select("td")[11].get_text().replace("DIMM", "").replace(" ", "")
