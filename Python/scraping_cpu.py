...
INSERT_SQL = """
  INSERT INTO cpu
  (product_id, url, product_name, price, release_date, core, freq, socket, gen)
  VALUES
  (%s, %s, %s, %s, %s, %s, %s, %s, %s)
"""

# 削除クエリ
DELETE_SQL = "DELETE FROM cpu"
...
def extract_data_from_soup(soup):
  ...
  for i in range(0, PAGE_CONTENT_COUNT):
    try:
      ...
      freq = elems[3*i+3].select("td")[9].get_text()
      socket = elems[3*i+3].select("td")[10].get_text().replace(' ', '').replace('\n','')
      core = elems[3*i+3].select("td")[11].select("a")[0].get_text().replace(' ', '').replace('\n','').replace('コア', '')
      thread = elems[3*i+3].select("td")[12].get_text()
      gen = elems[3*i+4].select("span")[2].select("a")[0].get_text()
      data = {
        "product_id"  : id,
        "url"         : link,
