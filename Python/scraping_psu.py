...
# スクレイピングするURL
PAGE_URL_BASE = "https://kakaku.com/pc/power-supply/itemlist.aspx?pdf_Spec001=1&pdf_pg="
...
INSERT_SQL = """
  INSERT INTO psu
  (product_id, url, product_name, price, release_date, capacity, formfactor, spec_80plus)
  VALUES
  (%s, %s, %s, %s, %s, %s, %s, %s)
"""

# 削除クエリ
DELETE_SQL = "DELETE FROM psu"
...
def extract_data_from_soup(soup):
  ...
  for i in range(0, PAGE_CONTENT_COUNT):
    try:
      ...
      capacity = elems[3*i+3].select("td")[8].get_text()
      formfactor = elems[3*i+3].select("td")[9].get_text()
      spec_80plus = elems[3*i+3].select("td")[10].get_text()
      data = {
        "product_id"  : id,
        "url"         : link,
        ...
        "capacity"    : capacity,
        "formfactor"  : re.sub('フォームファクタ：.+?規格：', '', formfactor),
        "spec_80plus" : spec_80plus
      }
    except:
      data = {
        "product_id"  : None,
